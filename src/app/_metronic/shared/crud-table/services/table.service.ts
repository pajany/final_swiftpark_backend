...
export abstract class TableService<T> {
  ...
  find(tableState: ITableState): Observable<TableResponseModel<T>> {
   ...
   // READ (Returning filtered list of entities)
   custlist(tableState: ITableState): Observable<TableResponseModel<T>> {
    const url = this.API_URL + '/custlist';
  ...
  findwithdate(tableState: ITableState, fromDate, todate): Observable<TableResponseModel<T>> {
    const url = this.API_URL + '/find';
    this._errorMessage.next('');
    return this.http.post<TableResponseModel<T>>(url, tableState).pipe(
      catchError(err => {
  ...
  public custfetch() {
    this._isLoading$.next(true);
    this._errorMessage.next('');
    const request = this.custlist(this._tableState$.value)
      .pipe(
        tap((res: TableResponseModel<T>) => {
          this._items$.next(res.items);
          this.patchStateWithoutFetch({
  ...
  public fetchwithdate(fromdate, todate) {
    this._isLoading$.next(true);
    this._errorMessage.next('');
    const request = this.findwithdate(this._tableState$.value, fromdate, todate)
      .pipe(
        tap((res: TableResponseModel<T>) => {
          this._items$.next(res.items);
          this.patchStateWithoutFetch({
  ...
  public patchdateState(patch: Partial<ITableState>) {
